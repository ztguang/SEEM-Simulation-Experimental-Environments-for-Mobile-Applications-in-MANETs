...
# Note: /system/etc/init.sh will be called 4 times: [init, bootcomplete, hci, hci]
case "$1" in
	netconsole)
	...
	bootcomplete)
		...
		# seem_init.sh is called for the second time
		/system/xbin/quagga/sbin/seem_init.sh
		#if [ ! -f /opt/init.txt ]; then
		#	echo "init.txt not exist, bootcomplete, in /system/etc/init.sh" >> /data/inittmp.txt
		#else
		#	echo "init.txt exist, bootcomplete, in /system/etc/init.sh" >> /data/inittmp.txt
		#fi
		;;
	hci)
		do_hci

		# comment hci in device/generic/common/init.x86.rc, line 126
		# do_hci is called for the second time
		#if [ -f /data/test.txt ]; then
		#	/system/xbin/quagga/sbin/seem_init.sh
		#fi

		# do_hci is called for the first time
		#touch /data/test.txt
		;;
	init|"")
		# seem_init.sh is called for the first time
		/system/xbin/quagga/sbin/seem_init.sh
		#if [ ! -f /opt/init.txt ]; then
		#	echo "init.txt not exist, init, in /system/etc/init.sh" >> /data/inittmp.txt
		#else
		#	echo "init.txt exist, init, in /system/etc/init.sh" >> /data/inittmp.txt
		#fi
